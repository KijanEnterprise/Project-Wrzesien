...
void html_pracownicy(vector<Pracownik> &Pracownicy)
{
	...
	plik<<"<tr><td>Lp.</td><td>ID</td><td>Imie</td><td>Nazwisko</td><td>Nr telefonu</td><td>Email</td><td>Adres</td><td>Wyplata</td><td>Nr rachunku</td><td>Data zatrudnienia</td></tr>"<<endl;
	for(int i=0;i<Pracownicy.size();i++){	
		char bufor[64];
		strftime(bufor,sizeof(bufor),"%d-%m-%Y",&Pracownicy[i].getDataZatrudnienia());
		plik<<"<tr><td>"<< i+1 <<"</td><td>"<< Pracownicy[i].getId() << "</td><td>" << Pracownicy[i].getImie() << "</td><td>" << Pracownicy[i].getNazwisko() << "</td><td>" << Pracownicy[i].getTelefon() << "</td><td>" << Pracownicy[i].getEmail() << "</td><td>" << Pracownicy[i].getAdres() << "</td><td>" << Pracownicy[i].getWyplata() << "</td><td>" << Pracownicy[i].getNrRachunku() <<  "</td><td>" << bufor << "</td></tr>"<<endl;
	}
...
void html_klienci(vector<Klient> &Klienci)
{
	...
	for(int i=0;i<Klienci.size();i++){	
		plik<<"<tr><td>"<< i+1 <<"</td><td>"<< Klienci[i].getId() << "</td><td>" << Klienci[i].getImie() << "</td><td>" << Klienci[i].getNazwisko() << "</td><td>" << Klienci[i].getTelefon() << "</td><td>" << Klienci[i].getEmail() << "</td><td>" << Klienci[i].getAdres() << "</td></tr>" << endl;
	}
	plik<<"</table></body></html>"<<endl;
	plik.close();
